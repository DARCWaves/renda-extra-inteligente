...
async function getDolarTempoReal() {
  try {
    const response = await axios.get("https://economia.awesomeapi.com.br/json/last/USD-BRL", { timeout: 8000 });
    const d = response.data.USDBRL;

    return {
      valor: `R$ ${Number(d.bid).toFixed(2)}`,
      numero: Number(d.bid),
    ...
    console.error("Erro dólar:", err.message);
    return null;
  }
}
